...
class Simulation:
    def __init__(self, carNum = 10, serverProcessRate=5, trafficRate=50):
        ...
        self.channel = None
        self.totalSimulateTime = 2 * 60 * 1000 # 1 min / unit - ms
        self.serverId = 10001
...
def find_avg_aoi(path):
    ...
    for file in glob.glob(path + '_*'):
        ...
        policy_aoi = {}
        for i, policy in enumerate(aoi_policy_dic):
            res = []
            for car in aoi_policy_dic[policy]:
                res += car
            policy_aoi[policyList[i]] = sum(res) / len(res)
...
def testTrafficRate():
    ...
    for i in [20, 33, 50, 100, 200]:
        res = {}
        for policy in policyList:
            print(policy)
            simulation = Simulation(carNum=10, serverProcessRate=5, trafficRate=i)
            simulation.runSimulate(policy)
            res[policy] = simulation.collectAOI()
...
def testUserNum():
    for i in [2, 4, 6, 8, 10, 12, 14, 16]:
        res = {}
        for policy in policyList:
            simulation = Simulation(carNum=i, serverProcessRate=5, trafficRate=30)
            simulation.runSimulate(policy)
            res[policy] = simulation.collectAOI()
...
plotAvgAOI('trafficRate.log', [10, 20, 33, 50, 100] , 'end user request sending interval (ms)', 'average AOI (ms)')
